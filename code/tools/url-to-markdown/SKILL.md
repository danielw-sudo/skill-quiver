...
model: any
description: >-
  Fetch any URL and convert it to clean markdown using Chrome CDP with site-specific adapters for X/Twitter, YouTube, and Hacker News. TRIGGER when: user asks to save a webpage or URL as markdown.
...
1. First run headless (default) unless there is already a reason to use interaction mode
...
6. Pass the constructed path to `--output`. Media files (`--download-media`) are saved into subdirectories next to the markdown file, keeping each URL's assets self-contained.
