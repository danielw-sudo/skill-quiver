...
model: any
description: >-
  Extract, analyze, and translate the core findings, methods, and practical implications of academic papers for personal use. TRIGGER when: user shares an arXiv link, paper URL, PDF, or asks to analyze a research paper.
...
输出结构依据 `$QUIVER_PATH/prompt/paper-reader/reference/template.org`。禁止参考 `~/Documents/notes/` 中已有论文文件的章节结构——旧文件可能使用过期模板。
...
判断标准：张图让人一看就抓住论文在做什么。不是所有论文都有——没有就跳过，不要硬找。
...
按 Denote 规范获取时间戳，读 `$QUIVER_PATH/prompt/paper-reader/reference/template.org`，写入 `~/Documents/notes/`。
